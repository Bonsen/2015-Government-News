...
public class ContentAdapter extends BaseAdapter {
	...
	public ContentAdapter(ArrayList<Map<String, Object>> listItem2,
		...
		UserStatus = sharedPreferences.getInt("UserStatus", 10);
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.time.setText(((String) listItem.get(position).get("time"))
				.substring(0, 10));
		holder.context.setText((String) listItem.get(position).get("content"));
		
		if(String.valueOf(listItem.get(position).get("status")).equals("0")) {
			System.out.println("你確定還是這裡？");
			holder.status.setText("待审");
			holder.status.setTextColor(R.color.blue);
		} else {
			System.out.println("之前在上一個，現在應該在這裡了啊");
			holder.status.setText("已审");
			holder.status.setTextColor(R.color.grey);
		}
		holder.tv_news_id.setText(String.valueOf(listItem.get(position).get(
				"id")));
		...
		holder.status.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (UserStatus == NewsCode.USER) {
					Toast.makeText(context, "用户没有审核权限！", Toast.LENGTH_SHORT)
							.show();
				} else {
					if (String.valueOf(listItem.get(position).get("status"))
							.equals("1")) {
						Toast.makeText(context, "不能重复审核！", Toast.LENGTH_SHORT)
								.show();
					} else {
		...
		convertView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// 在这里将数据发送过去就可以
				intent.putExtra("News",
						(HashMap<String, Object>) listItem.get(position));
				// 在意图中传递数据
				context.startActivity(intent);
			}
		});
